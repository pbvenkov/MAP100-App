...
FALLBACK_CRITERIA_REGISTRY = {
    "CONV-48.1": {"title": "Отсутствие виджета прямой онлайн-записи", "group": "Конверсия", "complexity": 2, "weight": 6.0, "descs": {"Обоснование_ОШИБКИ": "Отсутствие модуля прямой интеграции с системами онлайн-бронирования блокирует прием вечернего и ночного трафика."}},
    "PROF-10.3": {"title": "Отсутствие перечня ключевых услуг в тексте", "group": "Базовое заполнение", "complexity": 1, "weight": 4.0, "descs": {"Обоснование_ОШИБКИ": "В текстовом блоке карточки отсутствует структурированный перечень ключевых направлений работы."}},
    "REP-27.1": {"title": "Критический уровень рейтинга (Ниже 4.5)", "group": "Репутация", "complexity": 4, "weight": 2.5, "descs": {"Обоснование_ОШИБКИ": "Рейтинг ниже 4.5 — критическая зона. Клиенты жестко фильтруют компании на карте."}}
}
...
def calculate_client_potential(rating: float, score: float, lost_leads: int) -> Tuple[int, str, str]:
    stars = 0
    reasons = []
    if rating >= 4.7: stars += 2; reasons.append(f"отличная репутация ({rating})")
    elif rating >= 4.3: stars += 1; reasons.append(f"хороший рейтинг ({rating})")
    else: reasons.append(f"слабая репутация ({rating})")

    if score <= 65: stars += 2; reasons.append(f"критический балл ({score:.1f}/100)")
    elif score <= 85: stars += 1; reasons.append(f"средняя оптимизация ({score:.1f}/100)")
    else: reasons.append(f"карточка оптимизирована ({score:.1f}/100)")

    if lost_leads >= 20: stars += 1; reasons.append(f"потери -{lost_leads} лид/мес")
...
def get_gemini_insights(data: Dict[str, Any], logger: TerminalLogger) -> Dict[str, Any]:
    ...
    try:
        ...
        1. "score" (число 0-100).
        2. "pain_point" (текст): Одно предложение с самой грубой ошибкой.
        """

...
# ==========================================
# 1. СИСТЕМНЫЕ УВЕДОМЛЕНИЯ В TELEGRAM
# ==========================================
...
def save_audit_to_sheets(url, title, niche, total_score, lost_revenue, lpr_data=None):
    """Компактная фиксация сделки в CRM-воронке"""
    try:
        client = gspread.authorize(get_google_credentials())
...
def normalize_yandex_url(raw_url):
    """Преобразует любую ссылку Яндекса в канонический вид https://yandex.ru/maps/org/{ID}/"""
    url = raw_url.strip()
    ...
    # Поиск числового ID карточки (8-13 цифр)
    org_id_match = re.search(r'(?:org/|oid=|oid%3D|org\?oid=)(\d{8,13})', url)
    if not org_id_match:
        org_id_match = re.search(r'/(\d{8,13})', url)
        
    if org_id_match:
        return f"https://yandex.ru/maps/org/{org_id_match.group(1)}/"
        
    clean_url = url.split("?")[0].rstrip('/')
    clean_url = re.sub(r'/(reviews|gallery|features|menu)$', '', clean_url)
    return clean_url + "/"
...
def fetch_apify_data(yandex_url):
    ...
    while status not in ["SUCCEEDED", "FAILED", "ABORTED"]:
        if retries >= 35: 
            raise Exception("Таймаут сбора данных. Попробуйте еще раз.")
        time.sleep(4)
    ...
    if not dataset or not isinstance(dataset, list) or len(dataset) == 0 or not dataset[0].get('title'): 
        raise Exception(f"Яндекс не вернул карточку по адресу: {cleaned_url}. Проверьте доступность организации.")
...
# ==========================================
# 4. АЛГОРИТМЫ ОЦЕНКИ И АНАЛИТИКА
# ==========================================
...
Перепиши обоснование каждой ошибки под специфику этой ниши. Опирайся на упущенную выручку и барьеры для клиентов.
Ошибки:
{payload_text}
Верни строго JSON объект формата: {{"Код_ошибки": "Новый текст обоснования"}}"""
    try:
...
# ==========================================
# 6. ИНТЕРФЕЙС И ОСНОВНОЙ ПАЙПЛАЙН (STREAMLIT)
# ==========================================
...
tab_link, tab_file = st.tabs(["🌐 По ссылке (Яндекс Карты)", "📁 Из JSON файла (Ручной режим)"])

data_to_process = None
source_url = ""

with tab_link:
    url_input = st.text_input("Вставьте ссылку на карточку организации", placeholder="https://yandex.ru/maps/org/1063683074/")
...
# ОБРАБОТКА ДАННЫХ И ПОСТРОЕНИЕ АУДИТА
if data_to_process:

...
def generate_icebreaker(
    ...
    """Генерирует утвержденное первое сообщение руководителю (ЛПР)."""
...
def calculate_report_metrics(audit_data: Dict[str, Any]) -> Dict[str, str]:
    ...
    # Разбор ошибок для страницы 3 (с обновленной причиной №2: Витрина врачей)
    ...
    fail_2 = failures[1] if len(failures) > 1 else {
        ...
        "desc": "В карточке не оцифрованы профили врачей (фотографии, врачебный стаж, специализации). В медицине ключевое решение пациент принимает «на врача»: карточка проигрывает конкурентам с открытой командой."
    }
            ...
            "top_failures": [
                {
                    ...
                    "desc": "Пациенты в вечерние часы и при острой боли не хотят совершать звонок администратору. Карточка теряет от 60% вечернего трафика, отдавая записи клиникам с активным модулем записи."
                },
                {
                    "title": "Отсутствие витрины специалистов в профиле",

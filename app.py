...
def calculate_prof_rules(data):
    ...
    if isinstance(phones, list) and any("доб" not in str(p).lower() and len(re.sub(r'\D', '', str(p))) >= 10 for p in phones):
    ...
    if len(products) >= 10:
        ...
        if sum(1 for p in products if p.get('photoUrl') or p.get('imageUrl') or p.get('image')) / len(products) >= 0.8: 
            scores['PROF-11.2'] = True
        if sum(1 for p in products if p.get('price')) / len(products) >= 0.8: 
            scores['PROF-11.3'] = True
        if sum(1 for p in products if len(str(p.get('description') or '')) > 50) / len(products) >= 0.8: 
            scores['PROF-11.4'] = True
            
        cat_set = set(p['category'].get('name') if isinstance(p.get('category'), dict) else p.get('category') for p in products if p.get('category'))
        if len(cat_set) >= 2: 
...
def calculate_rep_rules(data):
    ...
    if isinstance(reviews, list) and len(reviews) > 0:
        try:
            rev_date = datetime.fromisoformat((reviews[0].get('date') or reviews[0].get('createdAt')).replace('Z', '+00:00'))
        ...
        for rev in last_20:
            ...
            if r_rate <= 3 and not reply: 
                unans_neg += 1
            if r_rate >= 4 and reply: 
                ans_pos += 1
        ...
        if len(owner_texts) >= 2:
            ...
            for t1, t2 in itertools.combinations(owner_texts[:10], 2):
                ...
                intersect_len = len(words1 & words2)
                if (intersect_len / union_len) > 0.8:
                    is_templated = True
                    break
...
def calculate_conv_rules(data):
    ...
    if data.get('posts') or data.get('news') or data.get('promos'): 
...
def calculate_act_rules(data):
    ...
    if not fresh and (data.get('stories') or data.get('storyUrls')): 
        fresh = True
...
def calculate_ai_rules(data):
    ...
    faq_str_list = []
    for q in faq_list[:3]:
        faq_str_list.append(f"Вопрос: {q.get('question')} Ответ: {q.get('answer')}")

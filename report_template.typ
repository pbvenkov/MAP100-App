...
#rect(width: 100%, fill: rgb("F8FAFC"), stroke: 0.5pt + rgb("CBD5E1"), radius: 4pt, inset: 9pt)[
  ...
  #text(size: 8.5pt, fill: rgb("334155"))[
    Прямо сейчас профиль скрыт от *[[DEV]]% целевых клиентов* вашего района. Из-за технических недочетов в оформлении карточки вы каждый месяц отдаете конкурентам локации около *[[LOST_LEADS]] [[AUDIENCE_DECLENSION]]*. \
    #v(2pt)
    *Парадокс репутации:* Высокий рейтинг подтверждает доверие постоянных клиентов. Но по общим поисковым запросам алгоритмы Карт скрывают карточку от новых жителей района с острой потребностью, перенаправляя их к соседям.
  ]
]
...
#v(4pt)
#text(size: 9pt, fill: rgb("475569"))[Почему потенциальные клиенты из вашего района уходят к конкурентам[[COMP_SAFE]]:]
#v(7pt)
...
#rect(width: 100%, fill: rgb("FFFFFF"), stroke: 0.5pt + rgb("CBD5E1"), radius: 4pt, inset: 10pt)[
  ...
  #text(size: 8.5pt, fill: rgb("475569"))[
    *Как видит пациент:* По прямому названию («[[TITLE]]») компанию найдут всегда. Однако первичные пациенты ищут не бренд, а решение проблемы: *[[SERVICE_EXAMPLE]]*. \
    *Что происходит в кассе:* Если в карточке нет структурированного прейскуранта с точными ценами, алгоритм исключает профиль из целевой выдачи и отдает платежеспособных клиентов карточкам с открытым прайсом.
  ]
]
#v(6pt)
